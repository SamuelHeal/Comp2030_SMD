function addSubtractMins(mins) {
        ...
        if(timestampDate.getTime() + (mins * 60 * 1000) <= Date.now()) {
        // Adds (or subtracts) 30 minutes (in milliseconds) to the timestamp
        let newTime = new Date(timestampDate.setTime(timestampDate.getTime() + (mins * 60 * 1000)));
...
// This is a copy of Arran's function from Reports to allow the whole button to be clickable and open the calender
